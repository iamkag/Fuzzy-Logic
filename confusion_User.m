classdef confusion_User
    methods (Static)
        function [c_matrix,Result,RefereceResult]= getMatrix(actual,predict,Display)
            ...
            %%http://www.dataschool.io/simple-guide-to-confusion_User-matrix-terminology/
            %%https://en.wikipedia.org/wiki/confusion_User_matrix
            ...
            % [c_matrix,Result,RefereceResult]= confusion_Usermat(actual,predict)
            %
            % %DIsplay off
            % % [c_matrix,Result,RefereceResult]= confusion_Usermat(actual,predict,0)
            %
            % %%
            % %single Class
            % n=100;m=1;
            % actual=round(rand(1,n)*m);
            % predict=round(rand(1,n)*m);
            % [c_matrix,Result]= confusion_Usermat(actual,predict)
            ...
            disp('confusion_User Matrix')
            disp(c_matrix_table)
            [Result,RefereceResult]=confusion_User.getValues(c_matrix);
            ...
            if Display
                if n_class>2
                    disp('Multi-Class confusion_User Matrix Output')
        ...
        function  [Result,RefereceResult]= getValues(c_matrix)
            ...
            if row~=col
                error('confusion_User matrix dimention is wrong')
            end
            n_class=row;
            switch n_class
                case 2
